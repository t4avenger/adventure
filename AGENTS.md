...
- Always run `make check` after changing code and before final response.
- If `make check` fails due to missing tools, run `make install-tools` and rerun `make check`.
